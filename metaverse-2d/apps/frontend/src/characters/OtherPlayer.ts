...
export default class OtherPlayer extends Player {
  ...
  preUpdate(t: number, dt: number) {
    ...
    if (animParts[1] === 'sit') {
      const shift = sittingShiftData[animParts[2]]
      if (shift) this.setDepth(this.depth + shift[2])
    }
    ...
    if ((vx !== 0 || vy !== 0) && body) body.velocity.setLength(speed)
...
Phaser.GameObjects.GameObjectFactory.register(
  ...
  function (
    ...
    ;(sprite.body as Phaser.Physics.Arcade.Body)
      .setSize(sprite.width * cs[0], sprite.height * cs[1])
      .setOffset(sprite.width * (1 - cs[0]) * 0.5, sprite.height * (1 - cs[1]) * 0.5 + 17)
    return sprite
  }
)
