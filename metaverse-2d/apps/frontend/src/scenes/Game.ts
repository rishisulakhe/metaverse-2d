...
export default class Game extends Phaser.Scene {
  ...
  registerKeys() {
    this.cursors = {
      ...
      ...(this.input.keyboard!.addKeys("W,S,A,D") as Pick<
        NavKeys,
        "W" | "S" | "A" | "D"
      >),
    };
  ...
  create(data: { network: Network; playerTexture?: string }) {
    ...
    const spawn = this.network.spawnPosition ?? { x: 705, y: 500 };
    // Clamp spawn into a reasonable pixel range so the player always lands
    // on-map even if the server sends coordinates outside the visible area.
    const safeX = Math.max(100, Math.min(spawn.x, 1400));
    const safeY = Math.max(100, Math.min(spawn.y, 1200));
    const texture = data.playerTexture ?? "adam";
    this.myPlayer = this.add.myPlayer(
      safeX,
      safeY,
      texture,
      this.network.mySessionId || "local",
    );
    // Set the local player's display name immediately so the nametag is visible
    const username = store.getState().user.username;
    if (username) this.myPlayer.setPlayerName(username);
    ...
    this.physics.add.overlap(
      ...
      this
        .handleItemSelectorOverlap as Phaser.Types.Physics.Arcade.ArcadePhysicsCallback,
      undefined,
      this,
    );
    ...
    // Replay players that were already in the room when we joined.
    // These were stored on Network during handleSpaceJoined because
    // scene.launch() is asynchronous — Game.create() runs AFTER the
    // PLAYER_JOINED events would have fired, so we replay them here
    // now that the listener is registered.
    for (const player of this.network.initialPlayers) {
      this.handlePlayerJoined(player, player.id);
    }
    this.network.initialPlayers = []; // clear after replay
  ...
  private handlePlayerJoined(newPlayer: IPlayer, id: string) {
    ...
    const other = this.add.otherPlayer(
      newPlayer.x,
      newPlayer.y,
      texture,
      id,
  ...
  private handleItemUserAdded(
    ...
    if (itemType === ItemType.COMPUTER)
      this.computerMap.get(itemId)?.addCurrentUser(userId);
    else if (itemType === ItemType.WHITEBOARD)
      this.whiteboardMap.get(itemId)?.addCurrentUser(userId);
  }
  ...
  private handleItemUserRemoved(
    ...
    if (itemType === ItemType.COMPUTER)
      this.computerMap.get(itemId)?.removeCurrentUser(userId);
    else if (itemType === ItemType.WHITEBOARD)
      this.whiteboardMap.get(itemId)?.removeCurrentUser(userId);
  }
