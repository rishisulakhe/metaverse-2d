...
async function hashPasswordOrNull(password?: string): Promise<string | null> {
...
spaceRouter.post("/", userMiddleware, async (req, res) => {
    ...
    const hashedPassword = await hashPasswordOrNull(parsedData.data.password);
...
spaceRouter.get("/all", userMiddleware, async (req, res) => {
    ...
    res.json({
        spaces: spaces.map((s) => ({
            ...
            hasPassword: !!(s as any).password,
        })),
    });
});

// Public listing of joinable spaces (for lobby screen).
spaceRouter.get("/public", async (_req, res) => {
    const spaces = await client.space.findMany({ take: 50 });
    res.json({
        spaces: spaces.map((s) => ({
            id: s.id,
            name: s.name,
...
spaceRouter.get("/:spaceId", async (req, res) => {
    ...
    res.json({
        ...
        hasPassword: !!(space as any).password,
